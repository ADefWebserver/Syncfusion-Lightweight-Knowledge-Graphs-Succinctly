...
No graph engine is added; the graph is these plain records/classes.
...
- Injected with `IDbContextFactory<SyncfusionHelpDeskContext>`.
- Queries with `AsNoTracking()` and `Include(t => t.HelpDeskTicketDetails)`.
- Exposes `Task BuildAsync(CancellationToken)` and a `LastSkippedEdges` build
  report property.
...
1. Create a `CreateDbContextAsync()` context and load all tickets with details
   using `AsNoTracking()`.
2. Initialize an empty `GraphDocument`, a `HashSet<string> nodeIds`, and reset
   `LastSkippedEdges = 0`.
3. For each ticket, add a Ticket node (`ticket:<Id>`) if its ID is new to the
   HashSet; add its Requester, Status, and Day nodes with case-insensitive
   de-duplication through the HashSet.
4. For each ticket detail, add a TicketDetail node (`detail:<Id>`) and its Day
   node, de-duplicated through the HashSet.
5. Confirm every intended node ID is present in `nodeIds` before proceeding.
   Nodes are now complete.
...
7. For each candidate edge, validate that both `From` and `To` exist in
   `nodeIds`. If both exist, add the edge; otherwise skip it and increment
   `LastSkippedEdges`.
8. Set `GeneratedUtc = DateTime.UtcNow`.
9. Write `graph.json` via `GraphFile.WriteGraphAsync`.
10. Compute counts by type and write `manifest.json` via
    `GraphFile.WriteManifestAsync`, setting `LastBuildUtc`.
...
Static class providing path resolution, loading, and **atomic** writes.

- `string GraphPath(GraphOptions)`, `string ManifestPath(...)`,
  `string MetadataPath(...)`, `string AuditPath(...)` resolve absolute paths
  under the configured output directory.
- `Task<GraphDocument?> LoadGraphAsync(...)` reads and deserializes
  `graph.json`.
- `Task WriteGraphAsync(GraphDocument, ...)`,
  `Task WriteManifestAsync(object manifest, ...)`, and
  `Task WriteMetadataAsync(object metadata, ...)` serialize and write.
...
- Additional derived paths are computed relative to `OutputPath`.
...
- by node type (`Dictionary<string, List<GraphNode>>`)
- by edge `From` value (`Dictionary<string, List<GraphEdge>>`)
- by edge `To` value (`Dictionary<string, List<GraphEdge>>`)
...
```mermaid
flowchart TD
    Reload[ReloadAsync] --> Read[GraphFile LoadGraphAsync]
...
- Create the configured output directory during startup (already present:
...
- After `app.Build()`, resolve `GraphStore` and call `ReloadAsync()` once so an
  existing snapshot is available immediately at startup.
